    ...
    if (sessionRef.current) {
      ...
      sessionRef.current.on(SessionEvent.SESSION_STATE_CHANGED, (state) => {
        ...
        if (state === SessionState.DISCONNECTED) {
          ...
          // Try to get room when state changes
          tryGetRoom();
        }
      });
      sessionRef.current.on(SessionEvent.SESSION_STREAM_READY, () => {
        setIsStreamReady(true);
        // Try to get room when stream is ready
        tryGetRoom();
      });
      ...
      // Initial attempt to get room
      tryGetRoom();
    }
  }, [sessionRef]);

  return { sessionState, isStreamReady, connectionQuality, room };
};
...
export const LiveAvatarContextProvider = ({
  ...
  const { sessionState, isStreamReady, connectionQuality, room } =
